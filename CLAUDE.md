...
6. **Apply the deferred review-tooling freshness fixes** (deferred from the S36 plugin-overlap analysis): `/ultrareview` → `/code-review ultra` across `bx-health` (SKILL.md + state-buckets.md), README "review tiers", and workflow.md; re-introduce the now-reinstated built-in `/simplify` into the Bucket A flow.
