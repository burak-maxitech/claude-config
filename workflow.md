...
| **SEO + GEO Audit** | `/seo-review` | Repo-wide SEO + Generative Engine Optimization audit for web projects. Fetches current best practices every run. Probes sitemap URL health. **Optional GSC integration via 4-state matrix**: BigQuery Bulk Data Export (Performance) + CSV exports (Page Indexing — CSV-only by Google limit). Auto-detects bq + gcloud ADC; falls back gracefully. Score `/100` tracked over time. |
...
**Decomposition:** Three parallel Sonnet subagents by default — `seo-technical` (crawlability, canonicals, mobile, hreflang, redirect config, perf signals, **sitemap URL health**), `seo-content` (titles, metas, headings, alt text, OG/Twitter Cards, internal linking), `geo-generative` (JSON-LD coverage, schema validation, llms.txt, E-E-A-T, semantic content patterns, AI-bot crawl access). **Plus a 4th `seo-gsc-insights` subagent when `.seo-data/gsc/` contains GSC CSV exports** (12 info-only sub-dims covering page-indexing reasons, CTR opportunities, position-band query opportunities, traffic orphans).

**Rubric:** 5 weighted dimensions summing to 100 (Technical SEO 25 / On-Page SEO 25 / Structured Data 20 / Generative Engine 20 / Performance 10). Fetched best practices may tune weights ±5 per dimension per run; sum delta always 0. Score breakdown by dimension + sub-dimension shown in the report. **GSC findings carry `score_impact: 0`** — orchestrator enforces this at consolidation. The score stays purely heuristic so `docs/seo-history.md` is comparable across runs whether GSC CSVs are present or not.

**GSC mode (opt-in, 4-state matrix):** two paths, both first-class — pick either or both:

- **Path 1 (RECOMMENDED for Performance):** BigQuery Bulk Data Export. Configure once (GCP project + dataset → `gcloud auth application-default login` → `.seo-data/gsc/config.yaml` with `project_id`/`dataset_id`/`location`). The orchestrator detects `bq --version` + ADC at runtime and runs 5 parallel SQL queries against `searchdata_url_impression` per run (~1-10 MB scanned, well under the 1 GB cost cap). Full history, no row caps, daily-fresh, 28 rich-appearance booleans.
- **Path 2 (REQUIRED for Page Indexing):** CSV exports in `.seo-data/gsc/indexing/`. Google's BigQuery export does NOT include indexing tables (verified across 5 official Google docs), so the 9-reason Page Indexing report is CSV-only. Also serves as Performance fallback when BigQuery isn't configured.

4-state matrix resolves which sources are active: **Full GSC** (BQ + indexing) / BQ-only / CSV-only / Indexing-only / CSV-Performance-only / **Heuristic-only** (no GSC at all → Section 1 banner). The folder is auto-`.gitignore`'d on first detection (config.yaml included). Setup banner is one-time + sentinel-gated and covers both paths in parallel callouts. New report Section 3 surfaces indexing coverage, top-impact GSC findings, CTR opportunities, position-band query opportunities, traffic orphans. **35-day git-history scan** annotates findings with `code_changed_since_gsc_window` when recent commits match affected paths — closes the 4-5 week GSC reporting-lag gap (recommendations no longer say "fix X" when commit abc123 already added it 18 days ago). Routing-rename + 404-cluster co-occurrence produces a `/plan-feature` bulk-redirect snippet (in `--plan` Phase 1).
...
**Output:** Single-headline score `/100` (with Δ when history present) + top-3 highest-impact opportunities (`[gsc]`-prefixed when GSC-sourced; unified rank includes traffic_weight × log10(impressions+1)). Section 1 detected-context + score breakdown. Section 2 findings per dimension. **Section 3 GSC Insights** (only when GSC mode enabled). Section 4 score-history table (when present). Section 5 next actions. Section 6 footer disclosure (sources fetched, sitemap probe results, CSV inventory + freshness, git-history-window status, subtotal-check arithmetic).

**Useful chain:** post-ship → `/code-cleanup` (delete dead pages) → `/seo-review` (audit what remains) → `/seo-review --plan` → `/plan-feature` per phase. Or ambient improvement → `/seo-review --fix` for safe scaffolds, then `/seo-review --plan` for the rest. **Quarterly GSC export refresh → `/seo-review`** for traffic-aware re-audit (re-export when major changes ship and you want to confirm GSC has caught up).
...
| | **`/seo-review` v3: BigQuery Bulk Data Export integration + 4-state matrix** (subagent count unchanged, allowed-tools += `Bash(bq:*), Bash(gcloud:*)`). Same-day pivot after v2 (CSV) shipped: user discovered GSC's Bulk Data Export to BigQuery and we verified across 5 official Google docs that the export is **Performance-only** (`searchdata_url_impression`, `searchdata_site_impression`, `ExportLog`) — Page Indexing data remains CSV-only by Google product limit. v3 is therefore a **hybrid**: BigQuery for Performance (no row caps, daily-fresh, 28 rich-appearance booleans) + CSVs for indexing. The 4-state matrix governs which sources are active: Full GSC / BQ-only / CSV-only / Indexing-only / Performance-CSV-only / Heuristic-only. **Auto-detection** of `bq --version` + `gcloud auth application-default print-access-token` at runtime; graceful fallback to CSV path when BQ unavailable. **No silent CSV fallback when BQ configured + fails** (locked decision 10 — surface errors loudly, skip Performance signal). **Byte-identical digest contract** ensures subagents are oblivious to source (BQ JSON output → cast INT64/FLOAT64 strings to Number → digest shape identical to CSV-sourced). New reference files: `bigquery-schema.md` (42-col `searchdata_url_impression` inventory + INT64/INTEGER alias note + schema drift handling), `bigquery-queries.md` (5 parametrized SQL templates Q1-Q5 with `--maximum_bytes_billed=1000000000` cost cap + anonymized-query filter), `bigquery-config-template.md` (flat single-level YAML enforced via Grep-based parser, no YAML lib needed). Setup banner rewritten as unified two-path callout (RECOMMENDED BigQuery + REQUIRED indexing CSVs). README template restructured around Path 1 / Path 2 with troubleshooting split CSV-vs-BQ. Section 1 gains "No GSC data" banner variant for heuristic-only mode. Plan-agent validation pass surfaced 8 blindspots / 4 simplifications / 4 ordering improvements — all adopted, including critical correctness fixes (`WHERE data_date BETWEEN ...` not `_PARTITIONTIME`; mandatory anonymized-query filter; flat YAML keys only). v3.x deferred: Snowflake/Redshift adapters, URL Inspection API for indexing, CrUX BigQuery dataset for Core Web Vitals — recorded in `bigquery-schema.md` deferred section so future sessions inherit context. |
