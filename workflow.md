...
| Aug 2026 | **`/bx:arch` review depth v2 + five first-run dogfoods** (bx v2.3.0–v2.8.0): catalog 23 → 54 entries adding OO/SOLID (`D`), concurrency (`C`), error safety (`E`) and scalability (`X`); cognitive complexity measured alongside cyclomatic; fifth scanner `arch-robustness`; calibrated finding contract (`finding-rubrics.md`); report opens with the top 3 architectural themes. Dogfooded end-to-end for the first time — the run found four defects six blind-rehearsal waves could not, because rehearsals feed scanners synthetic findings. `/bx:evolve`'s first full run likewise exposed a missing filter branch, and `claude plugin validate --strict` joined `/bx:save` Part 8, closing the S37 install-smoke-test leftover. `/bx:tests` then halted at Step 0 having resolved the repo's stack from a planted eval fixture — fixed by a shared `scan-exclusions.md` owner that both `/bx:arch` and `/bx:tests` read (governing stack *detection*, not just file scope), plus blast-radius severity and three-state `coverage_negatives`. Every skill run for the first time produced defects; none were findable by review. |
