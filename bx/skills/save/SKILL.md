...
effort: low
allowed-tools: Read, Edit, Write, Grep, Glob, Bash(git:*), Bash(ls:*), Bash(find:*), Bash(wc:*), Bash(awk:*), Bash(sort:*), Bash(cp:*), Bash(bash:*), Bash(claude:*), TaskList, TaskGet, AskUserQuestion, Agent
...
The final `ELSE` deliberately swallows the `v0`-with-a-CLAUDE.md case — `doc-schema.md`'s
"v0 mode routing" section is the authority there (including the why) and requires UPDATE,
never CREATE.
