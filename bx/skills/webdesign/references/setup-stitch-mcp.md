...
2. **`stitch-skills` installed?** — Confirm that the skill `stitch::code-to-design` is among available skills (callable via the Skill tool). If absent → the Google `stitch-skills` plugin is not installed.
...
- **Subsequent invocations:** if `.webdesign/.setup-shown` exists **and** the dependency is still missing, print a shorter one-liner reminder instead of the full banner:
  ```
  ⚙️ /bx:webdesign setup incomplete — [Stitch MCP | stitch-skills | both] still missing. Run the setup steps and try again.
  ```
  Then **stop** as before.
- **`--force-setup` argument:** if the user passes `--force-setup`, re-print the full banner regardless of whether the sentinel exists, then stop.
