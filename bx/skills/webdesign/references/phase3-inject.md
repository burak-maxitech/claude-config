...
2. **Start the dev server once (if `app_runnable == true`).** **First poll the port** — `curl -sf http://localhost:<port>/` — and if something already responds, **reuse it** rather than spawning a second server (Windows `KillShell` can orphan a prior `next dev` on the port; see Phase 1 Step 3.3). Otherwise, using `state.json["serve_cmd"]`, start the dev server in the background and wait for it to be ready (poll up to 30 s). Note the port it reports at startup; if it differs from `state.json["port"]`, update `state.json["port"]`. Keep the server running for the entire loop; stop it after Step 4 (or on early exit) with the `KillShell` tool using the background-shell ID from startup (never improvise `kill`/`taskkill`). Do not restart it per-page. Verification (`references/verification.md`) assumes the server is already running.
