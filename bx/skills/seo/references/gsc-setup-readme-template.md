...
This is the **template content** that the `/bx:seo` orchestrator auto-writes to `.seo-data/gsc/README.md` on first detection (when `.seo-data/gsc/` exists but `README.md` inside it doesn't).
...
This folder configures `/bx:seo`'s Search Console API integration.
Drop a `config.yaml` here (template below) and the next `/bx:seo` run
...
1. Run `/bx:seo` first WITHOUT this file — review the URL Inspection budget log + Section 3 findings.
...
(or just `config.yaml`). The next `/bx:seo` will note "Mode:
