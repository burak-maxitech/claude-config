# --fix Mode Allowlist (bx:seo)

`/bx:seo --fix` is deliberately the narrowest possible: **only mechanical, content-free, safe-by-construction additions** are eligible. Every other improvement routes to `--plan`.
