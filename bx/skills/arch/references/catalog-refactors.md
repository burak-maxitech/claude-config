# Catalog: Refactor Techniques (R-prefix)

Complexity-reducing refactor techniques. Consumed by the `arch-refactors` subagent.
Every finding must cite an entry by ID in `cite_catalog_entry`.

**Shared rules — languages tag, complexity-direction requirement, `--fix-eligible` policy,
gap proposals, and what is deliberately excluded — live in `catalog-rules.md`.** Read it
alongside this file; it is not restated here.
