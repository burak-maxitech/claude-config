...
5. **Remove** from CLAUDE.md each state section, `## Architecture Summary`, and — only when
   ...
   **Harness freshness declines** (both tiers): a tool decline of "unchanged since your last
   Read" reports *cache freshness*, not disk state. On a file this run wrote anything into
   (a section, the header block), it is NOT confirmation — Read the file back anyway; only an
   actual Read answers whether the write landed on disk. On a file nothing in this run wrote,
   it IS freshness confirmation rather than an obstacle.
