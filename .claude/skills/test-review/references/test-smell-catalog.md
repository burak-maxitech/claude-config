...
Each entry follows the same schema as the S-prefix simplification catalog in `architecture-review/references/refactor-catalog.md`. The `test-quality` subagent must cite an entry by ID in `smell_id`. Entries marked `--fix-eligible: true` may be auto-applied in `--fix` mode (still gated by per-finding diff preview).
...
- **Slow tests** — wall-clock cost without an obvious smell. Surface to user via `/code-health-advice` recommendation, but not as a finding here.
...
- **Orphaned tests, >3mo skipped tests, unused helpers, stale snapshots** — `/code-cleanup`'s `cleanup-styles-tests` covers these.
