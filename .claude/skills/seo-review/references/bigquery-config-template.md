...
| `site_url` | no | string | first `site_url` value in the data | The specific property to query when multiple properties export to the same dataset. v3 is single-property — if your dataset only exports one property, omit this and the orchestrator uses whatever's in the data. |
