...
| `upstream-changelog` | `/bx:evolve` | Researches Claude Code release notes and Anthropic changelog entries since the committed watermark date. Extracts new built-in commands, tool changes, hook additions, permission-model updates, and deprecations relevant to the bx plugin. |
| `upstream-community` | `/bx:evolve` | Researches community sources (forums, blogs, GitHub discussions) since the watermark for Claude Code usage patterns, emerging best practices, and workflow improvements not yet in official docs. |
| `upstream-docs` | `/bx:evolve` | Researches official Anthropic documentation updates (docs.anthropic.com, claude.ai/code docs) since the watermark. Extracts new capabilities, changed defaults, updated guidance on skills, agents, hooks, and MCP. |
