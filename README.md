...
> | `/seo-review` | whole web repo, SEO + GEO focus | SEO + Generative Engine Optimization audit. Fetches current best practices each run. Probes sitemap URL health (404/5xx/redirect-chain/slow). **Optional GSC CSV ingestion** (`.seo-data/gsc/`) for traffic-aware recommendations + 35-day git-history overlap to flag "may already be fixed" findings. Single score `/100` headline + top-3 priorities. Tracked over time in `docs/seo-history.md` (comparable across runs whether GSC CSVs present or not — score stays purely heuristic). Web projects only — rejects others silently. |
>
